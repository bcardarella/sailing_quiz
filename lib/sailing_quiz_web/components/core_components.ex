defmodule SailingQuizWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "fixed top-2 right-2 mr-2 w-80 sm:w-96 z-50 rounded p-3",
        @kind == :info && "bg-white text-slate-900 shadow-lg shadow-black/30 fill-cyan-900",
        @kind == :error && "bg-white text-slate-900 shadow-lg shadow-black/30 fill-rose-900"
      ]}
      {@rest}
    >
      <p :if={@title} class="flex items-center gap-1.5 text-sm font-semibold leading-6">
        <.icon :if={@kind == :info} name="hero-check-circle-mini" class="w-4 h-4 text-emerald-800" />
        <.icon
          :if={@kind == :error}
      ...
      <button type="button" class="absolute p-2 group top-1 right-1" aria-label={gettext("close")}>
        <.icon name="hero-x-mark-solid" class="w-5 h-5 group-hover:text-blue-700" />
      </button>
    </div>
    """
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SailingQuizWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SailingQuizWeb.Gettext, "errors", msg, opts)
    end
  end
